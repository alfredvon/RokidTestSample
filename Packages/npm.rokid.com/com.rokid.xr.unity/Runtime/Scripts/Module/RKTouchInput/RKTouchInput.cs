...
namespace Rokid.UXR.Module
{
    ...
    public class RKTouchInput : MonoSingleton<RKTouchInput>
    {
        ...
        public bool TryGetSmoothInsideTouchDeltaPosition(int index, out Vector2 delta)
        {
            delta = Vector2.zero;
        ...
        public bool TryGetSmoothInsideTouchDeltaPosition(int index, out Vector2 delta, ref Vector2 preDelta, ref int stationaryCount, ref int xInverseCount, ref int yInverseCount, int stationaryThreshold = 1, int inverseThreshold = 0, float inverseLengthDistance = 0.0f)
        {
            delta = Vector2.zero;
            if (index >= GetInsideTouchCount())
            {
            ...
            Touch touch = GetInsideTouch(index);
            switch (touch.phase)
            {
        ...
        public Vector2 GetInsideTouchDeltaPosition(int index)
        {
            if (index >= GetInsideTouchCount())
                return Vector2.zero;
            Vector2 delta = GetInsideTouch(index).deltaPosition;
            switch (UnityPlayerAPI.Instance.GetUnityScreenOrientation())
            {
        ...
        private void Update()
        {
            ...
            if (oldOperationFingerCount != GetInsideTouchCount())
            {
                oldOperationFingerCount = GetInsideTouchCount();
                elapsedTime = 0;
            }
            switch (fingerOperation)
            {
                case FingerOperation.None:
                    switch (GetInsideTouchCount())
                    {
                        case 1:
