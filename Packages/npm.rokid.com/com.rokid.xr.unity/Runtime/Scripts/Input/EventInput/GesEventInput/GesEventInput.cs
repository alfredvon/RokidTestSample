...
namespace Rokid.UXR.Interaction
{
    ...
    public class GesEventInput : MonoSingleton<GesEventInput>, IEventInput
    {
        ...
        /// Gesture module init callback
        /// </summary>
        [Obsolete(" Use OnInitializeGesModule instead")]
        ...
        public void SetActiveSleepJudge(bool active)
        {
            ...
            if (active == false)
            {
                ...
                OnHandSleep(HandType.LeftHand, false);

                rightHandSleep = false;
                rightHandSleepElapsedTime = 0;
                OnHandSleep(HandType.RightHand, false);
            }
        }
        ...
        private void JudgeHandSleep(HandType handType)
        {
            ...
            if (leftHandSleep && handType == HandType.LeftHand)
            {
                float sqrDis = Vector3.SqrMagnitude(curWritsPos - oriLeftHandWristPos);
                oriLeftHandWristPos = curWritsPos;
                if (GetHandClick(handType) || sqrDis > 4 * sleepWristPosThreshold * sleepWristPosThreshold)
                {
                    leftHandSleep = false;
                    leftHandSleepElapsedTime = 0;
                    OnHandSleep(HandType.LeftHand, false);
            ...
            if (rightHandSleep && handType == HandType.RightHand)
            {
            ...
            if ((leftHandSleep || rightHandSleep) && CanActiveHand(handType))
            {
                leftHandSleep = false;
                leftHandSleepElapsedTime = 0;
                OnHandSleep(HandType.LeftHand, false);
        ...
        // private float leftLongPressTime = 0, rightLongPressTime = 0, longPressTime = 0.3f;
        // private float leftClickTime = 0, rightClickTime = 0, doubleClickTime = 1.0f;
        // private int leftClickCount = 0, rightClickCount = 0;
        // private bool leftCalTime = false, rightCalTime = false;
        // private bool CanActiveHand(HandType handType)
        // {
        //     // if (InCameraTargetView(handType) && GetHandOrientation(handType) == HandOrientation.Back && GetHandPress(handType) && GetGestureType(handType) == GestureType.Pinch)
        //     // {
        //     //     if (handType == HandType.LeftHand)
        //     //         leftLongPressTime += Time.deltaTime;
        //     //     if (handType == HandType.RightHand)
        //     //         rightLongPressTime += Time.deltaTime;
        //     // }
        //     // else
        //     // {
        //     //     if (handType == HandType.LeftHand)
        //     //         leftLongPressTime = 0;
        //     //     if (handType == HandType.RightHand)
        //     //         rightLongPressTime = 0;
        //     // }
        //     // if (handType == HandType.LeftHand)
        //     //     return leftLongPressTime > longPressTime;
        //     // if (handType == HandType.RightHand)
        //     //     return rightLongPressTime > longPressTime;
        //     // return false;

        //     if (handType == HandType.LeftHand && leftCalTime)
        //         leftClickTime += Time.deltaTime;
        //     if (handType == HandType.RightHand && rightCalTime)
        //         rightClickTime += Time.deltaTime;

        //     if (InCameraTargetView(handType) && GetHandDown(handType) && GetGestureType(handType) == GestureType.Pinch && GetHandOrientation(handType) == HandOrientation.Back)
        //     {
        //         if (handType == HandType.LeftHand)
        //             leftClickCount++;
        //         if (handType == HandType.RightHand)
        ...
        //             leftCalTime = true;
        //         }
        //         if (leftClickTime < doubleClickTime)
        //         {
        //             if (leftClickCount == 2)
        //             {
        //                 leftClickTime = 0;
        ...
        //             rightCalTime = true;
        //         }
        //         if (rightClickTime < doubleClickTime)
        //         {
        ...
        {
            OnHandSleep?.Invoke(HandType.LeftHand, sleep && !lockInput);
            OnHandSleep?.Invoke(HandType.RightHand, sleep && !lockInput);
        }
