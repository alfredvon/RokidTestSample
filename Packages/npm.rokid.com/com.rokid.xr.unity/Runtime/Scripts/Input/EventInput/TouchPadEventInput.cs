...
namespace Rokid.UXR.Interaction
{
    ...
    public class TouchPadEventInput : MonoSingleton<TouchPadEventInput>, IEventInput
    {
        ...
        private void OnUpdate()
        {
            ...
            int touchCount = RKTouchInput.Instance.GetInsideTouchCount();
            Vector2 mouseDelta = new Vector2(Input.GetAxis("Mouse X"), Input.GetAxis("Mouse Y"));
            if (mouseDelta.magnitude > 0.0001f && touchCount == 0)
            {
                ...
                if (touchCount > 0)
                {
                    ...
                    switch (touch.phase)
                    {
                        ...
                        case TouchPhase.Stationary:
                            if (touchMove == false && touchCount == 1)
                            {
        ...
        {
            OnTouchPadSleep?.Invoke(sleep && !lockInput);
        }
