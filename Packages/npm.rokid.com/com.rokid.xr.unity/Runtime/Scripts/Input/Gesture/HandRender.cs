...
namespace Rokid.UXR.Interaction
{
    public class HandRender : AutoInjectBehaviour
    {
        ...
        private Dictionary<string, LineRenderer> skeletonLineDict = new Dictionary<string, LineRenderer>();
        ...
        void DrawSkeleton(Vector3[] skeletons, Quaternion[] rotations)
        {
            ...
            if (drawSkeletonLine)
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    {
                        skeletonLine = GameObject.Instantiate<LineRenderer>(this.skeletonLine);
